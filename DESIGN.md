...
Aplicado en la feature `011-planning-view`:

- **Cuatro estados visuales por celda, sin colores nuevos** (§8, condición 3) — vacía laborable, vacía no laborable con `--muted`, con horas sin pending, y con horas con pending. La última se distingue con un **borde punteado interno de 1px en `--secondary-foreground`**, no con un fondo distinto: el color no puede ser el único portador de la señal, y el número —siempre presente en `.font-data`— es la confirmación en texto.
- **Sobrecarga marcada con `--danger` y explicada por tooltip** (§8) — cuando la suma de todos los proyectos del dev supera 8h estrictas, la celda suma un **outline de 1.5px en `--danger` hacia adentro** (no fondo, para no chocar con el borde punteado de pending si ambos aplican). El tooltip lista los proyectos que están sumando, **incluidos los que están fuera del filtro actual**: sin eso, un PM que filtra por su cliente vería la sobrecarga sin explicación posible, y ese es exactamente el bug que la vista existe para prevenir (`plan.md` R-1). La marca hereda de `--danger` y no de `--attention` porque 8h es la jornada completa y superarla es la sobreasignación que ya define la rampa de la vista Mes.
