...
- [x] **T1.1** — `mondayOf(isoDate)` en `src/lib/calendar/range.ts`, más las tres ramas nuevas de `viewBounds` / `shiftDate` / `resolveRange` para `"planning"` (`plan.md` §8.2). El `date` en la URL **no se normaliza al lunes** para poder cargar `?view=planning&date=2026-05-14` (miércoles) y ver la semana que lo contiene. _DoD: unit tests de `mondayOf` con lunes, domingo, cambio de mes y cambio de año — la trampa histórica de `getUTCDay` sobre un `Date` en zona local (`plan.md` R-3)._
- [x] **T1.2** — `"planning"` en `calendarViewSchema` (`src/lib/validation/calendar.ts`). `DEFAULT_VIEW` sigue siendo `month`. _DoD: `parseCalendarParams` la acepta y `parseCalendarParams(?view=planning)` la devuelve; una vista basura sigue cayendo al default._
- [x] **T1.3** — `src/lib/calendar/planning.ts`: **funciones puras**.
  ...
  - _DoD: T3.1._
- [x] **T1.4** — `getDevDayLoad(supabase, range)` en `src/lib/calendar/query.ts` (`plan.md` §5): select mínimo (`dev_id, starts_at, ends_at`), `status in ('approved','pending')`, **sin filtros de entidad**. Reusa la misma silueta de `getDayLoad` pero sin pasar por `bookingsQuery` para dejar explícito que ningún filtro de UI la toca. _DoD: T3.2._
  - **El select dejó de ser el mínimo, y hay dos razones** (2026-09-11). `projectId` / `projectName` ya estaban, porque sin nombre el tooltip de AC-2.2 no puede nombrar el proyecto que el filtro esconde. Se sumó **`status`** por **R-5 de `plan.md` §10**: este total ignora los filtros y la grilla no, así que un PM mirando solo aprobadas puede leer una sobrecarga mayor que cualquier número en pantalla. `overloadContributions()` ahora devuelve `hoursApproved` / `hoursPending` por proyecto y el popover lo dice en palabras — que es lo que vuelve reconciliables las dos cifras en vez de parecer una contradicción.

## Phase 2 — UI

- [x] **T2.1** — `src/components/calendar/planning-view.tsx` (server component): la grilla completa (`plan.md` §7.1).
...
- [x] **T2.2** — `src/components/calendar/planning-cell.tsx` (client component): celda con popover.
...
- [x] **T3.1** — `renderPlanning()` en `src/app/(app)/calendar/page.tsx` como cuarta rama del switch de vista. `Promise.all` con `getBookingsInRange` (misma que la vista día — **no una `getPlanningBookings()` paralela**, `plan.md` §6) y `getDevDayLoad`. Reusa `emptyOrNoResults()` con `describeFilters()` sin cambios. _DoD: T4.1._
- [x] **T3.2** — Sumar `{ value: "planning", label: "Planificación" }` al array `VIEWS` de `src/components/calendar/calendar-toolbar.tsx`. Los botones anterior / siguiente / hoy funcionan solos gracias a `shiftDate("planning", ...)`. **La agrupación (`GROUPS`) no aparece en planning** — es cliente > proyecto > dev fijo, no un eje conmutable. Extender el `params.view === "day"` a la nueva vista sería mostrar un control que no hace nada.

## Phase 4 — Tests

- [x] **T4.1** — **Unit `plan.md` §12**:
...
- [x] **T4.2** — **Integración (CI)** — `tests/integration/planning-load.test.ts`, escrita el 2026-09-11. Las horas coinciden con las de `getBookingsInRange` sin filtros, y `getDevDayLoad` suma el proyecto de **otro PM** para una sesión de PM, una de admin y una de dev. **Lee las filas de vuelta y no el código de error**: la RLS filtra en silencio (ADR 0010), así que "no hubo error" no prueba nada — si la policy se cerrara, esto se vería como una suma más chica, nunca como una excepción.
- [x] **T4.3** — **E2E (CI)** — `tests/e2e/planning.spec.ts`, escrita el 2026-09-11. Cuatro semanas ancladas al lunes entrando por un miércoles, el salto de 28 días, filas por cliente/proyecto/dev, el feriado en `--muted` con sus horas igual, y el link al día con los filtros intactos. La aserción crítica de R-1 es la tercera: **con el filtro de cliente puesto, la celda sigue marcando sobrecarga y el popover nombra el proyecto que el filtro escondió**, con sus horas dichas como pendientes (R-5).
  - Va anclada a una ventana **fija** (lunes 2026-12-21) y no a la semana en curso, al revés que `calendar.spec.ts`: los feriados son escasos y una ventana que siga al reloj ejercitaría AC-1.4 unas pocas semanas al año. Navidad de 2026 cae viernes dentro de esa ventana, y 2026 y 2027 están cargados en `workdays.ts`.
  - La grilla ganó tres ganchos de test —`data-planning-grid`, `data-planning-day`, `data-workday`—, mismo criterio que `data-lane-header` en la vista día: engancharse a clases de Tailwind las convierte en API y se rompen cuando cambia el estilo.
