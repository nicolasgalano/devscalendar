...
- [x] **T7.1** — Ítem "Tickets" en `AppShell` (ícono `Ticket` de Lucide), en el grupo principal junto a "Calendario". Visible para cualquier rol autenticado; quien no sea miembro de ningún proyecto entra al empty state, no a un 403.
- [x] **T7.2** — `/tickets/page.tsx` server component. Filtros vía `parseTicketFilters(searchParams)`, query con `getTicketsList(filters, viewerId)`. `getTicketFacets()` alimenta los dropdowns (proyectos visibles + asignables activos, cacheado por request). Empty state triple: `NoResultsState` con filtros nombrados, `EmptyState` con CTA cuando el usuario es miembro pero sin tickets, y `EmptyState` sin CTA cuando no es miembro de ningún proyecto. `loading.tsx` con `TableSkeleton`. La acción primaria vive en el header cuando hay tickets y se muda al empty state cuando no (DESIGN.md §7).
- [x] **T7.3** — `<TicketList />` en `src/components/tickets/`. Tabla `Clave | Título | Estado | Prioridad | Asignado | Actualizado`. La clave va en `.font-data text-primary` porque es el identificador que el usuario aprende a leer. Filas clickeables por `<Link>` en la clave y en el título; el resto de las celdas heredan el hover de fila (`hover:bg-surface-hover cursor-pointer`). `updated_at` con helper propio `formatRelativeShort` — sin `date-fns`: no vale agregar la dep entera para 6 usuarios. Fecha absoluta en el `title`.
- [x] **T7.4** — `<TicketListFilters />` (client). Cada cambio navega a un href construido por `buildTicketsHref` (mismo patrón que `CalendarFilters` de `014`). Estado en la URL, `useOptimistic` para reflejar la elección hasta que el server rerenderee. Toggle "Incluir cerrados" con reset del status list al shortcut (`?includeClosed=1`). Search box `q` dispara al `blur` / `Enter`, no en cada tecla — un round trip por tecla es lo que un search "grande" evita, uno "chico" lo hace más rápido. `hasActiveTicketFilters` decide si aparece el "Limpiar filtros".
- [x] **T7.5** — `/tickets/[key]/page.tsx` server component. `getTicketByKey(key)`; si `null` → `notFound()` (los tres casos —parseo, proyecto no visible, ticket no visible— se colapsan en 404 sin distinguir, AC-6.1). Resuelve `role_in_project` con `rpc('role_in_project')` y los miembros activos con `getProjectMembers`. Delega el render en `<TicketDetail />`.
- [x] **T7.6** — `<TicketDetail />`: encabezado con `KEY` en `.font-data text-title text-muted-foreground` y el título en `text-title`. Meta con cliente/proyecto y creador en dos columnas. Controles inline de estado/prioridad/asignado, cada uno como `<Select>` que dispara PATCH con `useOptimistic` — el cambio se ve al instante y `router.refresh()` sincroniza; si el server rechaza, el optimista se descarta con el rerender. Los tres se deshabilitan por su cuenta según `canReassignTicket` / `canChangeTicketStatus` / `canEditTicketFields`. `<MarkdownViewer />` para la descripción. Botón "Editar" solo si `canEditTicket(...)` y abre el mismo `<TicketFormDialog>` del alta en modo `edit`.
- [x] **T7.7** — `<TicketFormDialog mode="create" | "edit" />` reusado por el CTA "Crear ticket" y por el botón "Editar" del detalle. Título / prioridad / asignado / descripción con `<MarkdownEditor />`; en modo `create` el proyecto se elige, en `edit` está fijo (por `enforce_project_key_immutable`). Al cambiar de proyecto en `create`, se recarga la lista de asignables desde `membersByProject` y se limpia el asignado que dejó de ser miembro — sin eso, el POST rebotaba con "El asignado no es miembro del proyecto". El CTA es `<CreateTicketButton>`: un client wrapper que abre el dialog cuando la URL trae `?new=1` y respeta `?projectId=` como preselección; cerrar el dialog reescribe la URL sin ese param, así el back button también cierra. Redirige a `/tickets/:key` al crear con éxito.
- [x] **T7.8** — `<TicketStatusBadge />` — icono + texto en `text-secondary-foreground`, `blocked` sube a `text-attention` (único estado que reclama acción), `done`/`cancelled` a `text-muted-foreground` (terminales). `<TicketPriorityBadge />` con las cuatro variantes existentes: `outline` (low), `priority-normal` (medium), `priority-high` (high), `danger` (critical). Sin tokens nuevos; la nota de `DESIGN.md` §3 sobre pasar a P0–P3 sigue abierta pero es de spec, no de este componente.

## Phase 8 — UI: gestión de miembros por proyecto

**Cambio de destino respecto de la spec original**: el panel se agregó como **tercer tab del workspace de proyecto** (`/projects/[key]/members`, feature 017), no como sub-vista de `/admin/projects/:id`. Motivo: el PM del proyecto también tiene que poder gestionar sus miembros (T8.3 lo pedía) y llevarlo al workspace en vez de admin lo hace contextual — el PM no tiene que ir a admin para tocar su propio equipo. `/admin/projects/*` sigue haciendo el ABM de proyectos.

- [x] **T8.1** — Ya cubierto en `015` P1 con `deriveProjectKey()` (T1.15b). El input explícito de `key` en el form de admin queda como iteración cuando alguien lo pida — hoy la derivación desde el nombre alcanza y evita la superficie de una key inválida.
- [x] **T8.2** — `<ProjectMembersPanel members addable projectId />` en `src/components/projects/`. Tabla activos + inactivos, PM primero con badge `(PM)`, `<Select>` inline para cambiar rol, botón "Desactivar"/"Reactivar" (desactivar pasa por diálogo de confirmación, reactivar es directo). Diálogo "Agregar miembro" con selector de usuario (activos no-ya-miembros) + selector de rol. Reusa `useSyncIndicator()` para el pill flotante durante los PATCH. **Al PM del proyecto no se le puede cambiar el rol ni desactivar desde este panel** — el trigger `autoadd_pm_as_lead` lo mantiene como `lead` y el `service_role` puede si hace falta.
- [x] **T8.3** — Guard **doble**: (a) el layout del workspace pasa `canManageMembers` al header, que esconde el tab si el usuario no es admin ni PM primario; (b) el `page.tsx` del tab redirige a `/board` cuando el URL directo llega con un usuario sin permiso. Sin (b), un contributor con la URL del portapapeles vería el panel pero los PATCH rebotarían del server con 403 — se lee como bug. Con las dos capas, la única forma de llegar y actuar es siendo admin o PM.
...
- [ ] **F6** — **Cobertura de UPDATE en integración.** El bug de `audit_ticket_events` (migration 15 lo arregla) rompía TODO UPDATE de ticket con `operator does not exist: jsonb - jsonb`, y no lo pescó CI porque los tests de integración de tickets cubrían INSERT y read-back de audit, pero no un UPDATE que dispare el diff genérico. Cuando se escriba Phase 9 (T9.2), sumar un caso que actualice `title` o `priority` para forzar el path del `jsonb_object_agg` del trigger reparado.
