...
| 024 | Planning: hoy en la última semana visible         | draft (spec) | 011          | fuera de spec original |
...
**`024` — alcance:** cuando la fecha activa de la vista **Planning** es hoy (botón "Hoy" o entrada sin `?date`), la grilla renderiza `[mondayOf(hoy) - 21, mondayOf(hoy) + 7)` — 3 semanas pasadas + la semana en curso, con hoy como la **cuarta fila** en lugar de la primera. Cambio de cómputo puro en `viewBounds` (`src/lib/calendar/range.ts`). Cero migration, cero UI directa, cero cambios en Day/Month/Year, cero cambios en el paso de navegación.
