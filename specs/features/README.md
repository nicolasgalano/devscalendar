...
**No se saldan sin OK explícito del usuario.** Dos son gate antes del primer
usuario real: **D-01** (`active = false` solo se aplica en la UI, así que
desactivar a alguien no le saca ningún permiso de API) y **D-06** (verificar en
el navegador los permisos de `/admin/*`). Una tercera, **D-09** (roles múltiples:
hoy nadie puede ser PM y admin a la vez), es gate antes de `010`, porque las
notificaciones salen hacia un `pm_id` que en ese caso miente. Las otras seis no
bloquean.
...
se quería. Quedan abajo, tachadas, porque el registro de *por qué* algo es como
...
`Q-O`) aparecieron durante el SDD, y las `Q-P*` son de `011`, que estrenó su
propio prefijo. Cada una vive con su enunciado completo en el `spec.md` de su
...
### Cerradas — features 001–006

Todas con la misma respuesta: **el default aplicado, confirmado el 2026-09-07**.
...
(R-2 de `006/plan.md` §9):** la prioridad juega al *crear*, no al *aprobar*. Dos
