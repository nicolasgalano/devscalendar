...
- [ ] **F7** — **Una reserva rechazada desaparece del calendario del PM por default.** `DEFAULT_STATUSES` de `003` es `pending, approved, displaced`: los estados terminales se ocultan para no llenar la grilla de tiempo que nadie va a trabajar, y `rejected` cayó de ese lado. Hasta `004` era inofensivo —nada podía llegar a `rejected` desde la app, solo desde el seed—; **`005` lo vuelve real**: el dev ya rechaza, y con las notificaciones diferidas a `010` el PM no tiene forma de enterarse salvo prendiendo un filtro que no sabe que necesita.
  - Choca además con `DESIGN.md` §8, que pone a rechazada y desplazada en la misma clase —"llevan color porque obligan al PM a reasignar"— mientras que `DEFAULT_STATUSES` muestra una y esconde la otra.
  - Salió al escribir T4.7: el E2E daba por hecho que el PM vería la rechazada y CI lo desmintió.
  - Tres salidas posibles, ninguna aplicada: sumar `rejected` a los estados por default; dejarlo oculto y que `010` lo resuelva por notificación; o un contador de rechazadas en la toolbar que las traiga con un clic. **Es decisión de producto, no de implementación.**
