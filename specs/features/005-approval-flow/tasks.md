...
- [x] **T1.3** — `pnpm db:types`. _DoD: `pnpm typecheck` limpio._ **Desbloqueada y cerrada el 2026-09-03 por B1:** apareció la PAT, el comando corrió, y el parche a mano resultó correcto. Lo de abajo queda como registro de por qué estuvo trabada.
...
- [x] **B1** — **Cerrado el 2026-09-03: apareció el token, `pnpm db:types` corrió, y el parche a mano de T1.3 estaba bien.** `responded_at` y `response_note` salen del generador exactamente como se habían escrito, en las tres secciones (`Row` / `Insert` / `Update`). Era la única duda que quedaba abierta de `005`.
  - Lo que **sí** difiere del archivo comiteado, y no tiene que ver con `005`: `PostgrestVersion` dice `14.15` y el generador devuelve `14.5`, y cinco paréntesis de más en los helpers genéricos del final. Es deriva de versión del CLI —el archivo comiteado lo generó otra—, no del esquema. Se absorbe en la próxima regeneración completa, que es `006` T1.4.
  - El diff completo parecía el archivo entero por CRLF contra LF. No lo es: git guarda LF y `core.autocrlf=true` convierte en el working tree, así que para git esa diferencia no existe. **Comparar siempre con `diff --strip-trailing-cr`** o el resultado no se puede leer.
  - Cómo se destrabó, por si hay que repetirlo en otra máquina: `pnpm exec supabase login --token sbp_…` —con `--token` no hace falta browser ni TTY, y también sirve la variable `SUPABASE_ACCESS_TOKEN`—, después `pnpm exec supabase link --project-ref gnasmpblvarluuwtjprq`, y ahí `pnpm db:types`. Es una PAT de cuenta, de Dashboard → Account Settings → Access Tokens; **ninguna de las keys del proyecto sirve**, la anon y la service_role son credenciales de PostgREST y esto va por la Management API.
  - `link` **no pidió la contraseña de Postgres**, y no la pide para nada de esto. Ver B2.
- [ ] **B2** — **`seed.sql` nunca se corrió contra el remoto.** `db push` aplica migrations, no el seed; para eso es `pnpm db:seed`, y con él se cierra la segunda mitad del DoD de T1.4 (dos corridas seguidas sin fallar ni duplicar). **Ya no está bloqueado: alcanza con la PAT de B1** (verificado el 2026-09-03). Una nota anterior de hoy decía que hacía falta además la contraseña de Postgres del proyecto, porque `db push` abre una conexión directa a la base. **Es falso, y conviene saber por qué:** el CLI moderno se autentica contra la base con el access token —el `db push --dry-run` lo muestra en su primera línea, "Initialising login role…"—, así que se conecta sin que nadie escriba una contraseña. El dry run confirmó de paso que el remoto está al día hasta `…0007` y que lo único pendiente es `…0008`. Ojo: el seed reescribe las reservas `…0051–005e` a la semana actual, así que se lleva puesto cualquier cambio hecho a mano sobre esas filas.
...
- [ ] **F5** — Q-N de `spec.md` §7 (se llamaba Q-F hasta el 2026-09-07): sin timeout de aprobación en el MVP. El recordatorio pasadas X horas necesita `010`.
- [ ] **F6** — `audit_log` solo lo lee el admin. Que el PM vea el historial de su reserva —quién la aprobó y cuándo— es de `010`.
- [x] **F7** — **Una reserva rechazada desaparecía del calendario del PM por default. Resuelto el 2026-08-31 sumando `rejected` a `DEFAULT_STATUSES`** (opción 1 de las tres, elegida por el usuario).
  - El origen: `003` definió el default como "ocultar los estados terminales", y eso puso a `rejected` del mismo lado que `cancelled`. Era inofensivo mientras nada en la app pudiera llegar a ese estado —solo el seed producía rechazos—; **`005` lo volvió real**, y con las notificaciones diferidas a `010` el PM no tenía forma de enterarse salvo prendiendo un filtro que no sabe que necesita.
  - Chocaba además con `DESIGN.md` §8, que pone a rechazada y desplazada en la misma clase —las dos que obligan al PM a reasignar— mientras `DEFAULT_STATUSES` mostraba una y escondía la otra.
  - **`cancelled` sigue oculto, y la regla ahora es otra:** no se esconde lo terminal, se esconde lo que el propio PM decidió. Una cancelación es suya, así que no hay a quién avisarle.
  - Salió al escribir T4.7: el E2E daba por hecho que el PM vería la rechazada y CI lo desmintió. El test quedó con la URL pelada a propósito, así que si `rejected` vuelve a salir del default, se cae de nuevo en vez de dejarlo pasar.
  - El unit test de `hasActiveFilters` que hardcodeaba la lista ahora la deriva de `DEFAULT_STATUSES` y la desordena: prueba que el orden no importa sin atarse a cuáles son los estados.
- [x] **F8** — **Bandeja para el admin: descartada; en su lugar, atajo al calendario filtrado** (decidido con el usuario el 2026-08-31).
  - El admin **no puede aprobar**, y no por UI: el trigger compara `auth.uid()` contra `dev_id` sin mirar el rol. Una "bandeja completa" para él sería una lista sin botones —un reporte con nombre de bandeja— y encima duplicaría lo que el calendario ya hace mejor: agrupación, seis filtros combinables y link compartible.
  - Quedó `Pendientes del equipo` en el nav, apuntando a `/calendar?status=pending`, visible solo para admin.
  - **Costó una precedencia nueva en el estado activo del nav.** `Calendario` y el atajo viven los dos en `/calendar`, y la determinación de activo de `DESIGN.md` §7 es por segmento de ruta: sin tocar nada, o se encendían los dos o el atajo no se encendía nunca. `findActive()` ahora prefiere el item que declara sus search params (`match`) sobre el que solo mira la ruta. El E2E lo verifica leyendo `aria-current` en los dos.
  - Lo que este atajo **no** contesta es "hace cuánto que está esperando", que es la pregunta de supervisión de verdad. Sigue siendo de `010` (F5, F6).
  - De paso quedó pineado en integración que **un admin no puede aprobar por otro**: el límite se discutió en voz alta y no lo cubría ningún test.
